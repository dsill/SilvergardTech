...
BEGIN
    ...
    BEGIN TRY
        ...
        --=====================================================================================--
        -- Prepare records to be dequeued & update ImportLog
        --=====================================================================================--

        --set current ImportStatusID (30=processing, 99=debug)
        set @ImportStatusID = case when @isDebug = 0 then 30 else 99 end
        ...
        --=====================================================================================--
        -- Create & populate ImportQueue temp table
        --=====================================================================================--

        --create and populate #tmp_ImportQueue, use cast to ensure datatypes
        ...
        ,       cast((case when a.[DataSourceUnknown] is NULL then 0 else 1 end) as tinyint)         as [DataSourceUnknown]
       ...
       --=====================================================================================--
       -- Create temp tables for core data
       --=====================================================================================--

        -- update #tmp_ImportQueue and generate CaseID from ClaimNumber and insert into #tmp_Case
        ...
        begin
            ...
            --=====================================================================================--
            -- Create Batch & Update ImportLog
            --=====================================================================================--

            --create BatchQueue records in [dbo].[ImportBatchQueue]
            select  @maxBatchID =   case when isnull(max(a.[BatchID]), 0) >= isnull(max(c.[BatchID]), 0) 
            ...
            -- update #tmp_ImportQueue with CaseID
            update  a
            ...
            ,       b.[DataSourceUnknown] = a.[DataSourceUnknown]
            ...
            --select a.*
            from    #tmp_Claim a
                    inner join [dbo].[Claim] b on a.[ClaimNumber] = b.[ClaimNumber] and b.[MasterClaimNumber] = a.[MasterClaimNumber]
                    inner join [dbo].[Case] c on a.[CaseCode] = c.[CaseCode]
                    inner join #tmp_ClaimContactMapping d on c.[CaseID] = d.[CaseID] and a.[MasterClaimNumber] = d.[MasterClaimNumber] and a.[ClaimNumber] = d.[ClaimNumber] 
            where   a.[isUpdate] = 1
            

            -- Insert Claim record if it does not already exist (existence based on [CaseCode], [CalimNumber], [MasterClaimNumber])
            insert into [dbo].[Claim]
            select  d.[ClaimContactID]
            ,       a.[MasterClaimNumber]
            ,       a.[ClaimNumber]
            ,       a.[AccountNumber]
            ,       a.[BeneficialOwnerName]
            ...
            --=====================================================================================--
            -- Update ImportQueue and ImportLog on completion
            --=====================================================================================--
        ...
        begin
            ...
            select '#tmp_ClaimContactMapping' as [table], * from #tmp_ClaimContactMapping
